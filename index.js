...
let parseText = (string) => string.toLowerCase().replace(/[0-9]+/g, '').replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g, "").replace(/[^a-zA-Z 0-9]+/g, '').split(' ');
...
let generatePoem1 = (corpus, numOfLines) => {
    let poem = '';
...
// var newPoem1 = generatePoem1(sampleText, 3);
// var newPoem2 = generatePoem2(sampleText, 3);
...
let getRandomWordExample = () => {
    let randomWordJSON, parsedJSON, wordExample, tempArr, tempSplice, tempIndex;
    ...
    // setTimeout(function extractExample() {
    //     parsedJSON.split(' ');
        
    // }, 6000);

    // setTimeout(function storeExample() {
    //     wordExample = parsedJSON['example'];
    // }, 4000);

    // setTimeout(function logExample() {
    //     console.log(typeof wordExample);
    // }, 4000);

    
};

console.log(getRandomWordExample());
